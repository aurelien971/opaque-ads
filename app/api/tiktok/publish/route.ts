import { NextResponse } from "next/server";

export const maxDuration = 60;

export async function POST(req: Request) {
  try {
    const [{ uidFromRequest }, { publishPost }, { adminDb }, { tiktokConfigured }] = await Promise.all([
      import("@/lib/server-auth"), import("@/lib/scheduler"), import("@/lib/admin"), import("@/lib/tiktok-server"),
    ]);
    if (!tiktokConfigured()) return NextResponse.json({ error: "TikTok keys are not configured yet." }, { status: 503 });
    const uid = await uidFromRequest(req);
    if (!uid) return NextResponse.json({ error: "Sign in first." }, { status: 401 });
    const { postId, ...opts } = await req.json();
    const snap = await adminDb().doc(`posts/${postId}`).get();
    if (snap.data()?.uid !== uid) return NextResponse.json({ error: "Not your post." }, { status: 403 });
    const publishId = await publishPost(postId, opts);
    return NextResponse.json({ publishId });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : String(e) }, { status: 500 });
  }
}
